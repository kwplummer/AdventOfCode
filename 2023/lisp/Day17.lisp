...
(defun moves (dp steps max-y max-x)
  (with-slots (y x dy dx) dp
    (let ((out (list)))
      (if (< steps 3) (push (list (1+ steps) (make-instance 'dirpos :y (+ y dy) :x (+ x dx) :dy dy :dx dx)) out))
      (when (= dy 0)
...
(defun ultra-moves (dp steps max-y max-x)
  (with-slots (y x dy dx) dp
    (let ((out (list)))
      (if (< steps 10) (push (list (1+ steps)
                                   (make-instance 'dirpos :y (+ y dy) :x (+ x dx) :dy dy :dx dx)) out))
      (when (>= steps 4)
        (when (= dy 0)
          (push (list 1 (make-instance 'dirpos :y (1+ y) :x x :dy 1 :dx 0)) out)
          (push (list 1 (make-instance 'dirpos :y (1- y) :x x :dy -1 :dx 0)) out))
...
(defun ultra-a-star (input)
  (let* ((parsed (parse-input input))
    ...
    (loop with counter = 0
          ...
          for dist = (first next) and pos = (second next) and steps = (third next)
          do (if (= 0 (mod (incf counter) 1000)) (format t "~:d: Best: ~:d, Peek: ~:d Graph Size: ~:d Queue Size: ~:d~%" counter best-distance dist (hash-table-count min-distance) (priority-queue:pqueue-length queue)))
          do (loop for (new-steps move) in (ultra-moves pos steps max-y max-x)
                   for y = (y-of move) and x = (x-of move) and dy = (dy-of move) and dx = (dx-of move)
                   for key = (list y x dy dx new-steps)
                   for new-distance = (+ dist (nnth y x parsed))
                   until (< best-distance new-distance)
                   ...
                   if (< new-distance old-distance)
                     do (setf (gethash key min-distance) new-distance)
                        (priority-queue:pqueue-push (list new-distance move new-steps) new-distance queue)
                        (when (and (= (1- max-y) y) (= (1- max-x) x) (>= new-steps 4))
                          (setf best-distance (min best-distance new-distance))
                          (format t "Finished building. New Best: ~:d~%" best-distance)))
          finally (return best-distance))))
(print (time (ultra-a-star (frog:get-advent-of-code-input 2023 17))))
