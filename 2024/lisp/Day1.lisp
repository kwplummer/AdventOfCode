...
(defun part-1 (input)
  (loop with left = nil
        with right = nil
        for (first second) in (->> input (str:lines)
                                (mapcar (lambda (s) (ppcre:register-groups-bind (left right) ("(\\d+)\\s+(\\d+)" s)
                                                      (list (parse-integer left) (parse-integer right))))))
        do (push first left) (push second right)
        finally (return (reduce #'+ (mapcar #'abs (mapcar #'- (sort left #'<) (sort right #'<)))))))
...
(defun part-2 (input)
  (let* ((parsed (->> input (str:lines)
                   (mapcar (lambda (s) (ppcre:register-groups-bind (left right) ("(\\d+)\\s+(\\d+)" s)
                                         (list (parse-integer left) (parse-integer right)))))))
         (second (mapcar #'alexandria:lastcar parsed)))
    (loop for (id _) in parsed summing (* id (count id second)))))
