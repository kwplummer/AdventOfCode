...
(in-package :advent)
(defclass* rule () (name rules)) ; Rule is a list of quads, First is the slot checked, second is the comparison, third is the value fourth is the destination rule
...
(defun parse-rule (line)
  (let* ((parts (str:split "{" (str:substring 0 -1 line)))
    ...
    (loop with parsed-rules = (list)
          for rule in rules
          if (= 1 (length rule))
            do (push (list nil nil nil (first rule)) parsed-rules)
...
(defun traverse-rule (rules rule-name ranges)
  ...
  (when (equal rule-name "A")
    (return-from traverse-rule (reduce #'* (mapcar (lambda (r)
...
(defun part-2 (file)
  (let* ((instructions (str:split frog:+double-newline+ file))
    ...
    (traverse-rule rules-by-name "in" (list
                                       (cons 0 4000)
                                       (cons 0 4000)
                                       (cons 0 4000)
                                       (cons 0 4000)))))
