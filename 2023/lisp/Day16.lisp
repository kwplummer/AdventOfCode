...
(defun run-maze (maze initial-beam)
  (loop with should-continue = t
        ...
        do (loop for beam in beams do (setf (gethash (list (y-of beam) (x-of beam)) lit) t))
           (setf beams (->> (mapcar (lambda (beam) (tick beam mirrors max-y max-x)) beams)
                         (alexandria:flatten)
                         (remove-if (lambda (beam) (or (< (x-of beam) 0) (< (y-of beam) 0)
                                                       (>= (x-of beam) max-x) (>= (y-of beam) max-y))))))
           (let* ((beam-positions (mapcar (lambda (b) (list b (format nil "~a" b))) beams))
             ...
             (setf should-continue (not (null new-beams))
                   seen-beams (nconc seen-beams (mapcar #'second new-beams))
                   beams (mapcar #'first new-beams)))
